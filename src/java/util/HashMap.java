...
public class HashMap<K, V> extends AbstractMap<K, V>
    ...
    static final int hash(Object key) {
        int h;
        return (key == null) ? 0 : (h = key.hashCode()) ^ (h >>> 16);// egg: ^ 异或：相同为0，不同为1 -->任何数同相同位数的全0数异或后为其本身// egg: >>> 忽略符号位右移，>> 保留符号位右移动// 00110101 00010010 01001100 11001010 (h)// 00000000 00000000 00110101 00010010 (h >>> 16)// 00110101 00010010 01111001 11011000 (^运算：高16位保持不变，新低16位为原高16位与原低16位进行异或运算-->减少低16位的hash碰撞)
    }
    ...
    final Node<K, V>[] resize() {
        ...
        int oldThr = threshold; // egg: threshold-->触发下次扩容时的大小 The next size value at which to resize (capacity * load factor).
        int newCap, newThr = 0;
        if (oldCap > 0) {
    ...
    static final class TreeNode<K, V> extends LinkedHashMap.Entry<K, V> {
         ...
         * comparing keys.
         */
        final TreeNode<K, V> find(int h, Object k, Class<?> kc) { // p:目标节点的hash值,k:目标节点的key,kc:目标节点key的运行时类型
            TreeNode<K, V> p = this;  // 当前节点
            do {
                int ph, dir;          // ph:当前节点的hash, dir:当前节点和目标节点的key值比较后的结果
                K pk;                 // pk:当前节点的key
                TreeNode<K, V> pl = p.left, pr = p.right, q; // pl:当前节点的左子节点,pr:当前节点的右子节点,寻找到的目标节点
                if ((ph = p.hash) > h)// 如果当前节点的hash值大于目标节点的hash值，则将左子节点赋给当前节点，进入下一次循环查询
                    p = pl;
                else if (ph < h)      // 如果当前节点的hash值小于目标节点的hash值，则将右子节点赋给当前节点，进入下一次循环查询
                    p = pr;
                else if ((pk = p.key) == k || (k != null && k.equals(pk)))// hash值相等且key值也相等则找到目标节点
                    return p;
                else if (pl == null) // hash值相等,但是key不等-->发生hash值碰撞-->树结构进一步分叉
                    p = pr;          // 若左子节点为空,将右子节点设置为当前节点，进入下一次循环查询
                else if (pr == null)
                    p = pl;          // 若右子节点为空,将左子节点设置为当前节点，进入下一次循环查询
                else if ((kc != null || // 若左右子节点都有值, 若key值也可以进一步比较（前提是客户端指定了kc）,则比较key值的大小
                        (kc = comparableClassFor(k)) != null) &&
                        (dir = compareComparables(kc, k, pk)) != 0)
                    p = (dir < 0) ? pl : pr;// 根据key值的比较结果决定将对应的子节点设置为当前节点,进入下一次循环查询
                else if ((q = pr.find(h, k, kc)) != null)// 若不具备key值比较的条件,则对右子节点树进行递归查询,直到hash值相等且key值也相等,则找到目标节点
                    return q;
                else // 右子节点树递归查询不到,则将左子节点赋给当前节点，进入下一次循环查询.
                    p = pl;
            } while (p != null);// 只要还存在当前节点,则一直循环查找,否则查找失败返回null
            return null;
        }
        ...
        static <K, V> TreeNode<K, V> rotateRight(TreeNode<K, V> root,
                                                 TreeNode<K, V> p) {// 参数root是根节点，p是被旋转的节点
            TreeNode<K, V> l, pp, lr; // 变量l是p的左节点、pp是p的父节点、lr是l右节点
            if (p != null && (l = p.left) != null) { // 判断p不等于null并把左节点赋值给l，如果l为null的话就不能右旋了
                if ((lr = p.left = l.right) != null) // 成上面性质c，并把值赋值给lr
                    lr.parent = p;
                if ((pp = l.parent = p.parent) == null) // 把p的父节点赋值给l的父节点，并把值赋值给pp，因为我们传入的参数G没有父节点，所以pp为null，为了满足红黑树的性质2(根节点为黑色)，把颜色设置成黑色
                    (root = l).red = false;
                else if (p == pp.right) // 是判断p是左节点还是右节点，如果是右节点则吧父节点的右节点设置成l，反之亦然
                    l = pp.right;
                else
                    l = pp.left;
                l.right = p; // 设置l的右节点为p，也就是P的右节点设置成G
                p.parent = l; // 设置p的父节点为l，也就是设置G的父节点为P
            }
            return root;
        }
